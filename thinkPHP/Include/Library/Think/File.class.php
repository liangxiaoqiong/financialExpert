...
function deletedir($dir)
{
	...
	if(@rmdir($dir))
	{
		echo "<script>alert(\"目录删除成功\"),window.location=\"http://localhost/FileContral/FileContral.php\";</script>";	
	}
	else
	{
		echo "删除失败".$dir;
...
if(@$_GET['delfile']!="") {
	...
	if(!file_exists($delfile)&&$exists!="1") {
		/*echo"<script>alert(\"删除成功\"),window.location=\"http://localhost/FileContral/FileContral.php\";</script>";*/
	} else {
...
if($_POST['dirname'])
{
	...
	if(is_dir($newdir))
	{
		...
		{
			/*echo"<script>alert(\"创建成功!\"),window.location=\"http://localhost/FileContral/FileContral.php\";</script>";*/
		}else {
...
if($_POST['editcontent'])
{
	...
	if($handle)
	{
		...
		//header("location:".$url);
		/*echo "<script>alert(\"编辑成功\");window.location=\"http://localhost/FileContral/FileContral.php\";</script>";*/
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="5">
			...
			<tr>
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="5"
					...
					<tr>
						<td><input type="hidden" name="path_f" value="<?php echo $editfile;?>"></input>
                        <textarea id="contents_edit" name="contents_file" style="width: 900px; height:600px; overflow-y: visible;"><?php if($edit_flag){ echo $contentfile;?><?php }else{ echo "no" ;}?> </textarea></td>
					</tr>
					<tr>
						<td><input style="background-color: gray" type="submit" name="editcontent" value="submit"></input></td>
					</tr>
					</form>
				</table>
				</div>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="5">
			...
			<tr>
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="5"
					...
					<tr>
					<td align="right"><a href="/"><font color="blue">返回首页</font></a></td>
					</tr>
				</table>
				</td>
			</tr>
